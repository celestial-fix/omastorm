...
if step build bash scripts/cargo.sh test --offline --locked --no-run; then
  ...
  lane alone check-bind check-link-plugin check-launcher check-theme check-weather-location check-map-tiles check-engine-install check-engine-release check-popover & lanes+=($!)
